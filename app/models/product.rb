...
class Product < Item
  ...
  def thumbnail_filename
    RAILS_ROOT + "/public/song_cache_square/#{self.id}"
  end
  public
      ...
      if self.composer_tag 
        if self.voicing_tags.size > 0 && self.name.present?
          ...
          if self.voicing_tags.size == 0
            raise "please check some voicing options first (like 'vocal solo' or SATB, or an instrument--use back button on browser to proceed) (if no voicing options match, please tell us!) selected were #{self.tags.map &:name}"
          end
...
#      self.code.upcase! # too ugly!
  ...
  def self.find_by_tag_id(tag_id, order_by = 'items.name ASC')
    tag = Tag.find(tag_id, :include => :products, :order => order_by) # including products.tags here is like 0.2s -> 2s but show_by_tags turns around and uses the tags!
